...
def employee_data_view(request):
    ...
    resp = '<h1>Employee No:{}<br>Employee Name:{}<br>Employee Salary:{}<br>Employee Address:{}</h1>'.format(
        employee_data['eno'], employee_data['ename'], employee_data['esal'], employee_data['eaddr'])
    return HttpResponse(resp)


def employee_data_jsonview(request):
    employee_data = {'eno': 100, 'ename': 'Sunny', 'esal': 1000, 'eaddr': 'Hyderabad'}
